    ...
    with journalist_app.test_client() as app:
        ...
        with InstrumentedApp(journalist_app) as ins:
            ...
            ins.assert_redirects(
                resp,
                '/admin/2fa?uid={}'.format(test_journo['id']))
...
def test_user_resets_totp(journalist_app, test_journo):
    ...
    with journalist_app.test_client() as app:
        ...
        with InstrumentedApp(journalist_app) as ins:
            resp = app.post('/account/reset-2fa-totp')
...
def test_admin_new_user_2fa_redirect(journalist_app, test_admin):
    with journalist_app.test_client() as app:
        _login_user(app, test_admin['username'], test_admin['password'],
                    test_admin['otp_secret'])
        with InstrumentedApp(journalist_app) as ins:
            resp = app.post(
                '/admin/2fa?uid={}'.format(test_admin['id']),
                data=dict(token=TOTP(test_admin['otp_secret']).now()))
            ins.assert_redirects(resp, '/admin/')
...
def test_http_get_on_admin_new_user_two_factor_page(
    ...
    with journalist_app.test_client() as app:
        _login_user(app, test_admin['username'], test_admin['password'],
                    test_admin['otp_secret'])
        resp = app.get('/admin/2fa?uid={}'.format(test_journo['id']))
...
def test_admin_sets_user_to_admin(journalist_app, test_admin):
    ...
    with journalist_app.test_client() as app:
        ...
        resp = app.post('/admin/edit/{}'.format(journo.id),
